...
export const ProductCard = ({ _id, name, code, imageUrl, description, category, blurhash }) => {
    ...
    return (
        <motion.div transition={{duration: 0.5}} animate={{opacity: 1}} initial={{opacity: 0}} exit={{opacity: 0}} className='rounded shadow-sm'>
            <Link to={`/products/${_id}`} className='text-decoration-none'>
                <section className="shadow-sm p-3 d-flex flex-column gap-2 mx-auto contentsec position-relative rounded">
